function [totalPower, convFlag, exitflag] = LPsolver3(matFix)
    %matFix = -1*ones(20,4);
    %matFix(1) = 1;
    ...
    CSRead = dlmread('../sourceData/CS_test_CSA_20.out');
    ...
    BSy = 0;
    N0 = 1e-16;
    tau = 200000; % Tx time per slot (2000ms)
    tier2NumSlot = 4;
    ...
    [sol,fval,exitflag] = linprog(objective,matConstraints,vecConstraints,Aeq,beq,lb,ub);
    %,[],options);
    ...
    % check convergence
    if exitflag == 1
        if length(find(Y>0.05 & Y<0.95)) == 0
            convFlag = 1;
        else
            convFlag = 0;
        end
